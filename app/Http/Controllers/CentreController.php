...
public function store(Request $request)
{
    ...
    for ($i=0; $i <sizeof($request->classes) ; $i++) { 
        $c = new Salle ;
        $c->capacite= $request->Capacite[$i];
        $c->num= $request->classes[$i];
...
public function getClassOfCentre($id)
{
    return Salle::where('centre_id',$id)->get();
}
}
